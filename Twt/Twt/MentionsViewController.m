...
//
/*
typedef NS_ENUM(NSUInteger, GetMentionsTimelineOption) {
    GetMentionsTimelineOptionGetAll,
    GetMentionsTimelineOptionOlderThan,
};
*/
...
- (void)viewDidLoad {
    ...
    [self getMentionsTimelineOption:RequestGETOptionGetAll tweetID:nil];
}
...
- (void)getMentionsTimelineOption:(RequestGETOption) option tweetID:(NSNumber *)tweetID {
    if (self.appDelegate.account.count) {
        NSURL *feed = [self.appDelegate requestURLWithOption:RequestGetMentionsTimelineURL stringParameter:nil];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (tableView == self.tableView && indexPath.row == self.tweetsArray.count) {
        ...
        LoadMoreCell *cell = [tableView dequeueReusableCellWithIdentifier:cellID];
        if (!cell) {
        ...
        NSArray *dataArr;
        if (tableView == self.tableView) {
            dataArr = self.tweetsArray;
        }
        else {
            dataArr = self.searchResultArray;
        }
...
- (void)tableView:(UITableView *)tableView willDisplayCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.row == self.tweetsArray.count - 1) {
        [self getMentionsTimelineOption:RequestGETOptionGetAll tweetID:(((TweetCell *)cell).tweetDict)[@"id"]];
    }
}
